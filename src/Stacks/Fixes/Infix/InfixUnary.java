        ...
        for(int i = 0; i < str.length(); i++){
            ...
            if(isNum(ch)) {
                ...
                if(ch == '(') opr.push(ch);
                    ...
                    // unary operator check after removing space
                    // first is - or + || - or + after ( || - or + after - or +
                    // then add 0 in front to handle unary
                    int j = i - 1;
                    while(j >= 0 && str.charAt(j) == ' ') j--; // Skipping if there are xtra spaces before the - or +
                    if (j < 0 || str.charAt(j) == '(' || str.charAt(j) == '+' || str.charAt(j) == '-') val.push(0);
