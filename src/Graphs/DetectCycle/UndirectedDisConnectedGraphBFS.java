...
public class UndirectedDisConnectedGraphBFS {
    ...
    private static boolean detectCycle(ArrayList<ArrayList<Integer>> adj){
        ...
        for(int i=0; i<n; i++){
            if(!visited[i] && isCycle(i, visited, adj)) return true;
        }

        return false;
    }

    private static boolean isCycle(int start, boolean[] visited, ArrayList<ArrayList<Integer>> adj){
        // make a queue and Add the start, and it's parent -1
