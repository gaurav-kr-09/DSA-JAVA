...
public class UndirectedConnectedGraphDFS {
    private static boolean detectCycle(int start, ArrayList<ArrayList<Integer>> adj){
        ...
        return DFS(start, -1, visited, adj);
    }

    private static boolean DFS(int start, int parent, boolean[] visited, ArrayList<ArrayList<Integer>> adj) {
        visited[start] = true;

        for(int neighbour: adj.get(start)){
            if(!visited[neighbour]){
                // agar cycle mila to return kar do nahi to agla branch check
                if(DFS(neighbour, start, visited, adj)) return true;
            }
